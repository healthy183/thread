package com.kang.concurrency.locks.cyclicBarrier;
...
public class CyclicBarrier {
    ...
    // 初始值为 parties;
    // parties - count = 已经到达的线程(正在等待的线程数量)
    private int count;
    ...
    private int dowait(boolean timed, long nanos)
        ...
        try {
            ...
            int index = --count; // 剩下还没达到等待栅栏的线程数量减一
            if (index == 0) {  // tripped  全部到达了
                boolean ranAction = false;
                try {
                    final Runnable command = barrierCommand;
                    if (command != null) //非空的话则调用回调函数
                        command.run();
                    ranAction = true;
                    nextGeneration();
                    return 0;
