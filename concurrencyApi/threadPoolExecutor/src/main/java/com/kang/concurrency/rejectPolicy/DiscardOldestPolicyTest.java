...
public class DiscardOldestPolicyTest {
    ...
    private static void simpleReject() throws InterruptedException {
        ...
        Thread.sleep(1000);
        threadPoolExecutor.execute(policyRunnable2);//give up the thread
        Thread.sleep(1000);
        threadPoolExecutor.execute(policyRunnable3);//give up the thread
        Thread.sleep(1000);
        threadPoolExecutor.execute(policyRunnable4);
        Thread.sleep(1000);
